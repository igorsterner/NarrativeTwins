    ...
    while batch_size * batch_idx <= 20000:
        ...
        for task in tqdm(asyncio.as_completed(tasks), total=len(tasks)):
            try:
                ...
                test_data[story_id]["extended_story"] = response_dict[
                    "extended_story_sentences"
                ]
                test_data[story_id]["narrative_twin"] = response_dict[
                    "retelling_sentences"
                ]
...
async def main():
    ...
    data_file = "data/rocstories/split/train_full.json"
    ...
    cache_path = "data/rocstories/split/cache.json"

    predictions_prompt = await run_experiment(
        all_data, extended_prompt, retelling_prompt, cache_path
    )
    out_file = "data/rocstories/split/train_narrativetwins.json"
