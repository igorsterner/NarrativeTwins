...
class NameDB:
    def __init__(self, path):
            ...
            self.data.append(
                {
                    "sex": sex,
                    "name": key,
                    "percent": sum(float(row["percent"]) for row in to_reduce),
                }
            )
        ...
        self.male_probs = [
            float(row["percent"]) for row in self.data if row["sex"] == "boy"
        ]
        self.female_probs = [
            float(row["percent"]) for row in self.data if row["sex"] == "girl"
        ]
    ...
    def random_name_with_sex(self, sex):
        if sex is not None:
            ...
            probs, max_val = (
                (self.male_probs, self.max_male_probs)
                if sex == "boy"
                else (self.female_probs, self.max_female_probs)
            )
